  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json({
        reply: `*${petName || "tu mascota"} te mira con curiosidad* No puedo hablar bien ahorita... intentalo despues!`,
        fallback: true,
      })
    }
    ...
    if (memories && memories.length > 0) {
      const memParts = memories.map((m: { key: string; value: string }) => `${m.key}: ${m.value}`)
...
Stats actuales:
- Hambre: ${stats?.hunger ?? 50}/100 ${(stats?.hunger ?? 50) < 25 ? "(tienes MUCHA hambre!)" : ""}
- Energia: ${stats?.energy ?? 50}/100 ${(stats?.energy ?? 50) < 25 ? "(estas MUY cansado!)" : ""}
- Felicidad: ${stats?.happiness ?? 50}/100 ${(stats?.happiness ?? 50) < 30 ? "(estas triste...)" : (stats?.happiness ?? 50) > 70 ? "(estas super feliz!)" : ""}
${memoryInfo}
...
- Responde SOLO en español`
    ...
    return NextResponse.json({
      reply: "Ups! Algo salio mal... intentalo de nuevo!",
      fallback: true,
    })
  }
}
